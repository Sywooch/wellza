        ...
        switch ($commodity_type) {
            case 'appointment':
                ...
                if ($appointment_id) {
                    ...
                    //echo 'here1';
                    //echo $data['upcoming'];die('...');
                    $id = $model->addAppointment('web',$data['upcoming']);
                }
                //echo $id;
                //die('..');
                if (!empty($id)) {
            ...
            case 'wellza_membership':
                ...
                //$model = new \common\models\UserMembership();
                $model = new \common\models\UserMembership();
                $plans_data = \common\models\MasterMembershipPlan::getPlan();
                $params = array();
                    
                if(!empty($plans_data) && $plans_data->duration == 'monthly') {
                    $params['plan_id'] = $plans_data->id;
                    $params['billing_cycle'] = $plans_data->duration;
                    $params['price'] = $plans_data->price;
                    $params['off_percent'] = $plans_data->off_percent;
                    $params['user_id'] = Yii::$app->user->id;
                    $params['plan_detail'] = 'Wellza Membership';
                    $params['start_date'] =  date("Y-m-d h:i:s");
                    $returndata = $model->add_membership($params);
                    
                    if(!empty($returndata)) {
                        $session['membership_id'] = $returndata;
                        $id = $returndata;
                        $qty = 1;
                        
                        $model = \common\models\UserMembership::findOne($id);
                        ...
                        if ($model) {
                            ...
                            \common\models\Cart::saveSessionData($id, $qty, 'wellza_membership');
        ...
        if (!empty($data['id'])) {
            ...
            switch ($commodity_type) {
                ...
                case 'wellza_membership':
                    ...
                    if ($model) {
                        Yii::$app->cart->remove($model);
                        \common\models\Cart::saveSessionData($id, '', 'wellza_membership');
                        return json_encode(['success' => true, 'count' => count(Yii::$app->cart->getPositions())]);
                    }
                break;
            }
