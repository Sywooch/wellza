...
class Packages extends BasePackages {
     ...
     * @return Slots|Null
     */
    public static function getPackageSlots($category_id = null, $subcategory_id = null,$appointment_minutes = null) {
        $returndata = array();
        
        if(!empty($appointment_minutes)) {
             $query = self::find()
                ...
                ->andWhere('package_services.minutes_duration = :minutes_duration', ['minutes_duration' => $appointment_minutes])
                ->all();
        } else {
             $query = self::find()
                ->select('`packages`.`service_id`,`packages`.`parent`,`packages`.`preparation_status`,`package_services`.`start_time`,`package_services`.`end_time`,`package_services`.`minutes_duration`,`package_services`.`service_price`')
    ...
    /**
     * Get the all the data related with a package
     * @param type integer package_id
     * @return package|null
     */
    public static function getPackageData($package_id = null) {
        return self::find()->ByPackage($package_id)->One();
    }
