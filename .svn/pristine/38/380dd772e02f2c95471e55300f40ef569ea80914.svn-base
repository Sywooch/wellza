        ...
        time: {
            render: function(e) {
                ...
                for (var r = "", d = 0; d < t.length; d++) {
                    ...
                    switch (d > 0 && (r += " "), o) {
                        case "%Y":
                            e._time_format_order[3] = d, r += "<select style='display:none'>";
                            for (var l = s.getFullYear() - 5, h = 0; 10 > h; h++) r += "<option value='" + (l + h) + "'>" + (l + h) + "</option>";
                            r += "</select> ";
                            break;
                        case "%m":
                            e._time_format_order[2] = d, r += "<select style='display:none'>";
                            for (var h = 0; 12 > h; h++) r += "<option value='" + h + "'>" + this.locale.date.month_full[h] + "</option>";
                            r += "</select>";
                            break;
                        case "%d":
                            e._time_format_order[1] = d, r += "<select style='display:none'>";
                            for (var h = 1; 32 > h; h++) r += "<option value='" + h + "'>" + h + "</option>";
                            r += "</select>";
                            break;
                        case "%H:%i":
                            e._time_format_order[0] = d, r += "<select class='selectpicker'>";
                            var h = 540,
                                _ = s.getDate();
        ...
        if (!this._lightbox) {
            ...
            for (var s = 0; s < a.length; s++) {
                ...
                if (n) {
                    ...
                    var r = "";
                    var classes = 'dhx_wrap_section test_'+s;
                    a[s].button && (r = "<div class='dhx_custom_button' index='" + s + "'><div class='dhx_custom_button_" + a[s].button + "'></div><div>" + this.locale.labels["button_" + a[s].button] + "</div></div>"), this.config.wide_form && (t += "<div class='"+classes+"'>");
