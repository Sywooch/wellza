...
class Appointment extends BaseAppointment implements \yz\shoppingcart\CartPositionInterface {
    ...
    public function fields() {
        return ArrayHelper::merge(parent::fields(), [
                    ...
                    'last_name',
                    'address',
                    'review_count','rating','is_rated',
                    'is_member', 'appointments','service_id','sub_service_id','is_favourite'
        ]);
    }
    ...
    public static function getApppointmentData($id = null, $status = null, $type = null) {
        ...
        $select = "`appointment`.`id`,`appointment`.`customer_id`,`appointment`.`provider_id`,`appointment`.`package_id`,"
                ...
                . "`user`.`first_name`,`user`.`last_name`,`user`.`profile_image`,`user`.`address`,`service`.`category_id` AS service_id,sub_service.category_id AS sub_service_id,"
...
//            $query = self::find()->byStatus($status, $id)->byDate($now_array[0]);
        ...
        $query = $query->select($select)
                ...
                ->leftJoin('services AS  sub_service', '`packages`.`service_id` = `sub_service`.`category_id`');
        ...
        if (!empty($dataProvider) && empty($type)) {
            ...
            foreach ($dataProvider->getModels() as $appointment) {
                ...
                if ($appointment['profile_image_thumb'] != '' && file_exists($appointment['profile_image_thumb'])) {
                    $profileImage_thumb = $url . '/' . $appointment['profile_image_thumb'];
                } else {
                    $profileImage_thumb = \yii\helpers\Url::to('@web', true) . Yii::getAlias('@defaultimg');
                }
                ...
                $appointment['review_count'] = ReviewRating::getReviewCount(['provided_to'=>$appointment['provider_id']]);
                $appointment['rating'] = ReviewRating::avgRating(['provided_to'=>$appointment['provider_id']]);
                $appointment['is_member'] = UserMembership::isMember($appointment['customer_id']);
                $appointment['is_rated'] = ReviewRating::isRated(['provided_to'=>$appointment['provider_id'],'provided_by'=>$appointment['customer_id'],'appointment_id'=>$appointment['id']]);
                $appointment['is_favourite'] = boolval(FavouriteList::find()->byProviderId($appointment['provider_id'])->byStatus('Active')->byUserId($appointment['customer_id'])->count());
    ...
    /**
     * get all the upcoming appointment for the provider
     */
    public static function getProviderApppointmentData($id = null, $status = null, $type = null) {
        $result = [];
        $expression = new Expression('NOW()');
        $now = (new \yii\db\Query)->select($expression)->scalar();
        $now_array = explode(' ', $now);
        ...
        $select = "`appointment`.`id`,`appointment`.`customer_id`,`appointment`.`provider_id`,`appointment`.`package_id`,"
                ...
                . "`appointment`.`postal_code`,`appointment`.`personal_information`,`appointment`.`appointment_price`,`appointment`.`appointment_duration`,"
                . "`appointment`.`appointment_time`,`appointment`.`appointment_date`,`appointment`.`appointment_endtime`,"
                . "`user`.`first_name`,`user`.`last_name`,`user`.`profile_image`,`user`.`profile_image_thumb`,`user`.`address`,`service`.`category_id`,"
                . "`service`.`category_name`,`service`.`category_image`";
        if ($type == 'Upcoming') {
          
            $query = self::find()->ByProviderStatus($status, $id);
        } else {
            $query = self::find()->ByProviderStatus($status, $id);
        }
        ...
        if (!empty($dataProvider) && empty($type)) {
            ...
            foreach ($dataProvider->getModels() as $appointment) {
                ...
                $appointment['sub_category_name'] = $category['category_name'];
                $appointment['profile_image'] = $profileImage;
                $appointment['profile_image_thumb'] = $profileImage_thumb;
                $appointment['category_image'] = $categoryImage;
    ...
    /**
     * Get the appointment cancel by the provider for a particular client
     */
    public static function cancelAppointment($id, $status) {
        $appointment = self::find()->byId($id)->one();
        if (!empty($appointment)) {
            $appointment->status = $status;
            return $appointment->update(false);
        } else {
            return false;
        }
    }
